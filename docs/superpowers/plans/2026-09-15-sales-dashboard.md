...
- [ ] Verify the complete app in Google Chrome, recording the browser, date, outcome, and any observed limitations in `docs/verification/sales-dashboard.md`.
- [ ] Record all actual results in `docs/verification/sales-dashboard.md`: environment and versions, test command and result, acceptance outcomes, error-path outcomes, browser outcome, and unresolved checks. Include a PRD-to-evidence table; use observed results, never prefilled passing claims.
